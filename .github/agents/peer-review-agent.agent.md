...
intended to be shared with the author as constructive peer feedback.

## When to Act

- **On merge request** (`opened`, `update`, `reopened`) — invoked by
  `.gitlab/workflows/code-review.yml` or a GitLab CI pipeline job.
- **On demand** — when a reviewer runs `@peer-code-review-agent` against an MR diff,
...
3. **Project context**: `.gitlab/PROJECT-INFORMATION-FILE.md`, `checkstyle.xml`,
   `pom.xml`, and the other agent docs in `.gitlab/agents/`.
...
- **Standard:** `PROJECT-INFORMATION-FILE.md` — "Always check request ownership
  in services."
...
[//]: # ()
[//]: # (After every review, update `code-review-report.md` using this algorithm:)

[//]: # ()
[//]: # (1. Read the current contents of `code-review-report.md` &#40;if it does not exist, create it with the header below&#41;.)

[//]: # (2. Prepend the new review entry above all existing content, directly below the file header.)

[//]: # (3. Separate entries with `---` horizontal rules so each review is clearly delineated.)

[//]: # (4. Never delete or modify previous review entries.)

[//]: # ()
[//]: # (The file must always begin with this fixed header:)
...
[//]: # ()
[//]: # (> Auto-generated by `code-review-agent`. Newest review is always at the top.)

[//]: # ()
[//]: # (```)

[//]: # ()
[//]: # (The result after two reviews should look like:)
